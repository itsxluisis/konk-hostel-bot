...
- V1.2 (guarda de fechas pasadas en get_availability) construida el 24-sep-2026 — rama `v1-2-guarda-fechas`. Diseño final (tras ver un log real): `get_availability` rechaza cualquier `checkin_date` anterior a hoy sin llamar a Cloudbeds ni avisar a Telegram, y responde con un texto dirigido al modelo (calendario de hoy a +14 días + siguiente aparición del mismo mes/día) para que recalcule, en vez de "corregir" la fecha él mismo. Detalle en `memory.md`.
