...
- V1 DESPLEGADA el 24-sep-2026 11:15 (merge `219db47`). Secretos rotados por Luis el 24-sep (`VAPI_SECRET` nuevo en EasyPanel y cabecera `x-vapi-secret` en cada tool y en el Server del assistant; `VAPI_API_KEY` regenerada y actualizada en EasyPanel y GitHub Actions). V1.1 (redacción del proxy + fusión del PATCH en servidor + escape HTML + contadores de rotación en `/health`) en curso.
- V1.2 (guarda de fechas pasadas en get_availability) construida el 24-sep-2026 — rama `v1-2-guarda-fechas`. Diseño: `get_availability` rechaza cualquier `checkin_date` anterior a hoy sin llamar a Cloudbeds ni avisar a Telegram. El rechazo empezó siendo un texto dirigido al modelo (calendario + siguiente aparición del mismo mes/día) para que recalculara; el auditor encontró (ALTA) que el prompt ordena leer la respuesta de esta tool entera y tal cual, así que ese texto se habría leído al huésped — corregido sin tocar el prompt: ahora es una frase hablada natural ("Perdona, no he entendido bien las fechas. Hoy es \<día> de \<mes>. ¿Qué día quieres entrar y qué día salir?"), sin ISO, sin año, sin "error". Commit C del mismo día antepone las fechas consultadas (en español) a toda respuesta de disponibilidad que sí llega a Cloudbeds, para que un error en una fecha futura también sea audible. Detalle en `memory.md`.
- V2a (acceso reconocido) construida el 24-sep-2026 — rama `v2a-acceso-reconocido`, sin mergear. Nuevo `src/guest-lookup.js` (`findStayByPhone`) cruza el teléfono de quien llama contra Cloudbeds (llegadas hoy/mañana + alojados hoy, sin API de Vikey ni canal de contacto nuevo) para añadir la reserva a `get_current_date` (tope 2,5s) y avisar "🚨 URGENTE — NO PUEDE ENTRAR" en `report_incident` cuando la categoría es acceso y el huésped llega hoy pasadas las 15:00 o ya está alojado. `/health` → `guestLookup`. Sin tocar `vapi/system-prompt.md`. Detalle en `memory.md`.
- V2b (botón hecho en avisos de llamar) construida el 25-sep-2026 — rama `v2b-boton-hecho` (desde `v2a-acceso-reconocido`), sin mergear. Los avisos "📞 LLAMAR" de Telegram llevan un botón "✅ Hecho"; al pulsarlo (cualquier miembro del chat autorizado, no solo el jefe) se edita el mensaje con quién y a qué hora y se quita el teclado — sin estado nuevo ni recordatorios. Detalle en `memory.md`.
