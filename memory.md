...
- **Campos de Cloudbeds usados:** teléfono de `guestPhone`/`guestCellPhone`/`phone`/`cellPhone`, a nivel de reserva y de cada huésped en `guestList[]` (Cloudbeds no documenta un nombre de campo único estable; si falta en todos, ese huésped simplemente no puede coincidir — no rompe nada). Habitación de `rooms[]` (`roomName`/`roomTypeName`/`roomID`/`roomNumber`) o, si Cloudbeds no la da como array, del mismo `roomID`/`roomNumber` que ya usa `getReservationsByDate`; sin ninguno de los dos, `rooms: null` (nunca se inventa una habitación).
...
- **`/health` → `guestLookup: { matches, misses, errors, lastAt }`** (solo números y fecha, nunca datos de huésped).
- **Decisiones en casos límite:** "a nombre de" usa el nombre de la RESERVA (no necesariamente el huésped exacto de `guestList` cuyo teléfono coincidió) — en un hostel donde solo quien reserva suele tener teléfono registrado, coincide casi siempre. El "prefijo de país" al comparar teléfonos es "todo lo que sobra antes de los últimos 9 dígitos" (no una tabla ITU real): sirve para NO cruzar dos países que compartan por casualidad los últimos 9 dígitos, no para validar el prefijo en sí — sin impacto en el caso normal (mismo número, formatos distintos). Sin campo de habitación → "sin asignar" en el aviso de Telegram, `rooms: null` en el objeto devuelto.
- **Sin verificar contra Cloudbeds real** (la tarea lo pidió así: tests con Cloudbeds mockeado, sin tocar secretos). Antes de mergear, probar con una llamada real de Luis que dispare `get_current_date` y `report_incident` con una reserva de verdad, y revisar en logs si `guestPhone`/`guestList[].guestPhone` es de verdad el campo que trae el teléfono.
- **Rama `v2a-acceso-reconocido`, sin mergear, sin push.** No se tocó `vapi/system-prompt.md` ni `.github/workflows/`. Tests sin red (axios interceptado, mismo patrón que `cloudbeds-errors.test.js`/`get-availability-dates.test.js`): `test/guest-lookup.test.js` (30 casos) y `test/vapi-guest-recognition.test.js` (18 casos, arranca el servidor real). `npm test`: 21 archivos, todos en verde.
